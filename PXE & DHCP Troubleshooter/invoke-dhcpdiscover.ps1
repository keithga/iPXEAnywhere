...
param(
    ...
    $RequiredArchitectures = @(
        @{ id = 0x07; description = 'x64 UEFI' }
        @{ id = 0x0b; description = 'ARM 64-bit UEFI' }
        @{ id = 0x10; description = 'x64 uefi boot from http' }
        @{ id = 0x13; description = 'arm uefi 64 boot from http' }

    ),
    #FUTURE
    $OptionalArchitectures = @(
        @{ id = 0x00; description = 'x86 BIOS' }
        @{ id = 0x06; description = 'x86 UEFI' }
        @{ id = 0x0a; description = 'ARM 32-bit UEFI' }
        @{ id = 0x0f; description = 'x86 uefi boot from http' }
        @{ id = 0x12; description = 'arm uefi 32 boot from http' }
    ),
...
write-verbose "Create UDP Port 67 Broadcast, and send Message "
$CliEndPoint = [Net.EndPoint](New-Object Net.IPEndPoint ($([Net.IPAddress]::Broadcast,67)))
$Message = New-DhcpDiscoverPacket -MacAddressString $MacAddressString -SecondsElapsed 4 -ProcessorArchitecture $ProcessorArchitecture -Option60String $Option60String
